...
#              Alexis AMTMANN - Mathis BESSON - Sasha BRETON - Clément DROUET              #
...
# Setup pygame/window ---------------------------------------- #
mainClock = pygame.time.Clock()                        # declaration de l'horloge interne
...
data_carte_selectionnee = {"chemin" : "map_plage/plageV2.tmx", "musique" : "assets/curiosidad.wav", "zoom_min" : 0.32, "zoom_max" : 3 , "depart" : [256,549]}
son_selectionne = son_ok                                  # memoire du son
joueur_selectionne = j2                                   # memoir du nombre de joueurs
difficulte_selectionne = facile                           # memoire de la difficulte
...
def tirage_couleurs():
    """définie 2 couleurs aleatoire parmis 10 (tirage sans remise) dont 1 chance sur 50 d'obtenir une formule 1"""
    ...
    # chaque joueur a une chance sur 100 d'avoir la formule 1, il y a donc 2/100 d'avoir la formule 1 dans la partie soit 1/50
    couleurs_choisies = random.sample(liste_couleurs, 2)
    if p == 2 : #joueur 2 formule 1
        tortue_2_img_grand = pygame.image.load('assets/tortue_special.png')
        tortue_2_img = pygame.transform.scale(tortue_2_img_grand, (30,30))
        tortue_1_img_grand = pygame.image.load('assets/tortue_' + couleurs_choisies[0] + '.png')
        tortue_1_img = pygame.transform.scale(tortue_1_img_grand, (25,25))
    elif p == 1: #joueur 1 formule 1
        tortue_2_img_grand = pygame.image.load('assets/tortue_' + couleurs_choisies[0] + '.png')
        tortue_2_img = pygame.transform.scale(tortue_2_img_grand, (25,25))
        tortue_1_img_grand = pygame.image.load('assets/tortue_special.png')
        tortue_1_img = pygame.transform.scale(tortue_1_img_grand, (30,30))
    else:
        tortue_2_img_grand = pygame.image.load('assets/tortue_' + couleurs_choisies[0] + '.png')
        tortue_2_img = pygame.transform.scale(tortue_2_img_grand, (25,25))
...
def parametres():
    ...
    parametres = screen.blit(bg_parametre_img, (44, 207))
    ...
    while True:
        ...
        if click:
            ...
            # volume ----------------------------------------------------------
            if bouton_son.collidepoint((mx, my)) and son_selectionne == son_ok:
...
def pre_jeu():
    ...
    while True:
        ...
        if keys[pygame.K_LEFT] or keys[pygame.K_RIGHT] or keys[pygame.K_DOWN] or keys[pygame.K_UP]: # "connecter" le joueur2
            screen.fill(fond_jeu, (225,543,209,133))
            screen.blit(tortue_1_img_grand,(255,497))
            check_j1 = True
        if keys[pygame.K_z] or keys[pygame.K_q] or keys[pygame.K_s] or keys[pygame.K_d]: # "connecter" le joueur 1
            screen.fill(fond_jeu, (16,543,209,133))
            screen.blit(tortue_2_img_grand,(30,497))
            check_j2 = True

        if check_j1 and check_j2: # afficher le boutton "commencer"
            bouton_commencer = screen.blit(commencer, (56,330))


        if click :
            if check_j1 and check_j2 and bouton_commencer.collidepoint((mx,my)):
                jeu()
    ...
    while True:
        ...
        if click:
            if carte_plage.collidepoint((mx, my)):
                if img_carte_selectionnee != carte_plage_img_grand: # si n'est pas deja selectionne > changer le dict/musique/image
                    data_carte_selectionnee = {"chemin" : "map_plage/plageV2.tmx", "musique" : "assets/curiosidad.wav", "zoom_min" : 0.32, "zoom_max" : 3 , "depart" : [256,549]}
            ...
            if carte_nuit.collidepoint((mx, my)):
                if img_carte_selectionnee != carte_nuit_img_grand:
                    data_carte_selectionnee = {"chemin" : "IceMap/IceMap.tmx", "musique" : "assets/nuit.wav", "zoom_min" : 0.45, "zoom_max" : 2,"depart" : [436, 863]}
            ...
            if carte_foret.collidepoint((mx, my)):
                if img_carte_selectionnee != carte_foret_img_grand:
                    data_carte_selectionnee = {"chemin" : "Foret/Forestmap.tmx", "musique" : "assets/soupe.wav", "zoom_min" : 0.32, "zoom_max" : 2,"depart" : [625, 551]}
...
def jeu():
    ...
    walls, finish, checkpoint1, checkpoint2, checkpoint3, checkpoint4, boost, slow, zone1, zone2, zone3, zone4 = [], [], [], [], [], [], [], [], [], [], [], []

    for obj in tmx_data.objects: # creation de listes regroupant les positions des hitbox de chaque type
        ...
        if obj.type == "zone1":
            zone1.append(pygame.Rect(obj.x, obj.y, obj.width, obj.height))

        if obj.type == "zone2":
            zone2.append(pygame.Rect(obj.x, obj.y, obj.width, obj.height))

        if obj.type == "zone3":
            zone3.append(pygame.Rect(obj.x, obj.y, obj.width, obj.height))

        if obj.type == "zone4":
            zone4.append(pygame.Rect(obj.x, obj.y, obj.width, obj.height))
    ...
    zoom = data_carte_selectionnee["zoom_min"]
    x_overlay = -55
    ...
    while True: # afficher un decompte a partir de 5, faire descendre le timer et zoomer jusqu'au joueurs simultanement
        t_now = (t_depart-datetime.datetime.now()-datetime.timedelta(seconds = 4)) # 9-4 = 5

        t_4 = datetime.timedelta(-1,54,0,0,59,23) # delai avant zoom (commence à 3 secondes)
        ...
        screen.blit(overlay, (60, x_overlay)) # barre du timer
        objets.texte("0:00.00", 24, (0,0,0), screen, 90, x_overlay+8, font)
        objets.texte("0/2", 24, (0,0,0), screen, 285, x_overlay+8, font)

        objets.texte(str(t_now)[15:16], 80, (0,0,0), screen, "center", 250, font)
    ...
    while boucle:
        ...
        if joueur_selectionne == j2:
            ...
            if (sqrt((tortue1.position[0] - tortue2.position[0])**2) * zoom) >= 200 and zoom > data_carte_selectionnee["zoom_min"]:
                ...
                zoom_x += 0.02 if zoom_x > 1 else 0.01
            if (sqrt((tortue1.position[1] - tortue2.position[1])**2) * zoom) >= 500 and zoom > data_carte_selectionnee["zoom_min"]:
        ...
        lag = (60/mainClock.get_fps()) # gestion du lag genere par la grande taille des cartes
        if joueur_selectionne == j2: # gestion des different cas nb_de_joueurs/difficulte
            if difficulte_selectionne == difficile:
                ...
                tortue1.update_pos_simple(keys, lag*(2+boost1+slow1), tortue_1_img)
                tortue2.update_pos_simple(keys, lag*(2+boost2+slow2), tortue_2_img)
        ...
        # COLISIONS ==========================================================================================
        for sprite in group.sprites(): # collision avec les murs
            if sprite.rect.collidelist(walls) > -1:
                sprite.colisions(walls)
                bruit = pygame.mixer.Sound("assets/collision.wav")
                bruit.play()

            if sprite.rect.collidelist(zone1) > -1 and sprite.delta_y > 0 : # detection de deplacement en sens inverse
                    sprite.image = interdit
            if sprite.rect.collidelist(zone2) > -1 and sprite.delta_x < 0 :
                    sprite.image = interdit
            if sprite.rect.collidelist(zone3) > -1 and sprite.delta_y < 0 :
                    sprite.image = interdit
            if sprite.rect.collidelist(zone4) > -1 and sprite.delta_x > 0 :
                    sprite.image = interdit

            if sprite.rect.collidelist(finish) >= 0: # gestion du compte-tour et de la memoire du chrono
                if sprite.check == [1,1,1,1]:
                    if sprite.delta_y <= 0: # verification du passage dans le bon sens (vers le haut)
                        sprite.tour += 1
                    if sprite.tour == 2 and sprite == tortue1:
                        chrono1 = t_now
                    elif sprite.tour == 2 and sprite == tortue2:
                        chrono2 = t_now
                sprite.check = [0,0,0,0] # reinitialisation des checkpoints

            if sprite.rect.collidelist(checkpoint1) > -1: # gestion des checkpoints
                sprite.check[0] = 1
            if sprite.rect.collidelist(checkpoint2) > -1:
                sprite.check[1] = 1
            if sprite.rect.collidelist(checkpoint3) > -1:
                sprite.check[2] = 1
            if sprite.rect.collidelist(checkpoint4) > -1:
                sprite.check[3] = 1

        boost1 = 0
        boost2 = 0
        for sprite in group.sprites():
            if sprite.rect.collidelist(boost) > -1: # gestion des zones de boost
                if sprite == tortue1:
        ...
        slow2 = 0
        for sprite in group.sprites():
            if sprite.rect.collidelist(slow) > -1: # gestion des zones de ralentissement
                if sprite == tortue1:
        ...
        group.draw(screen) # afficher la carte et les joueurs (faire sauter le if a l'interieur)

        if joueur_selectionne == j2:
            deriere = False if tortue1.position[1]*zoom >= 70 and tortue2.position[1]*zoom >= 70 else True # faire disparaitre le timer
            if min(tortue1.tour, tortue2.tour) == 2: boucle = False # terminer la course
        if joueur_selectionne == j1:
            deriere = False if tortue1.position[1]*zoom >= 70 else True
            if tortue1.tour == 2: boucle = False

        if deriere == False:
            if x_overlay <= 14: # terminer l'animation d'avant course
                x_overlay += 1
            ...
            objets.texte(str(compte_tour) + "/2", 24, (0,0,0), screen, 285, x_overlay+8, font)

        if t_now < datetime.timedelta(seconds = 2): # terminer l'animation d'avant course
            objets.texte("GO !", 80, (0,0,0), screen, "center",250, font)
    ...
    while True:
        ...
        if map_layer.zoom < data_carte_selectionnee["zoom_min"]: # attendre que la carte soit completement dezoome
            fin_jeu()
            #if joueur_selectionne == j2:
                #pre_questions()
            #else: questions()
        ...
        if x_overlay > -55: # retracter le timer
            x_overlay -= 1


        if joueur_selectionne == j2:
            group.center(((tortue1.position[0] + tortue2.position[0])/2, (tortue1.position[1] + tortue2.position[1])/2))
        else:                          # centrer la carte
            group.center(tortue1.position)
        ...
        screen.blit(overlay, (60, x_overlay))
        objets.texte(str(t_now)[3:10], 24, (0,0,0), screen, 90, x_overlay+8, font)
        objets.texte("2/2", 24, (0,0,0), screen, 285, x_overlay+8, font)
...
def pre_questions():
    pygame.display.set_caption('Clash Turtle - A vos neurones !')
    screen.fill(fond_jeu)
    ...
    while True:
        ...
        keys = pygame.key.get_pressed()
        if keys[pygame.K_LEFT] or keys[pygame.K_RIGHT] : # connecter les joueurs
            screen.fill(fond_jeu, (225,543,209,133))
            screen.blit(tortue_1_img_grand,(255,497))
            check_j1 = True
        ...
        if check_j1 and check_j2:
            bouton_commencer = screen.blit(commencer, (56,330)) # afficher le bouton "commencer"
...
def questions():
    ...
    choisies = random.sample(liste_questions, 5) # tirer au sort (sans remise) un nombre de questions (5)
    malus1, malus2 = 0,0

    for question in choisies:
        pygame.display.set_caption('Clash Turtle - Questions  |  '+ str(choisies.index(question)+1)+'/5') # afficher la progression sur 5
        pygame.mixer.music.load("assets/compte_a_rebours_15s.wav")
        pygame.mixer.music.play(1)
        screen.fill(fond_jeu)
        i = 0
        # reinitialisaser les positions
        if joueur_selectionne == j2:
        ...
        while i < 800: # hauteur de la fenetre
            for event in pygame.event.get():
                if event.type == QUIT:
                    pygame.quit()
            ...
            screen.fill(fond_jeu)
            screen.fill((132,244,129), (0,0,450,i)) # sablier
            objets.texte("Question", 40, (0,0,0), screen, "center", 55, font)
            objets.texte(question["q"], 30, (0,0,0), screen, "center", 149, font) # question en cours
            ...
            i += (800/(15*mainClock.get_fps())) # increment à ajouter pour que le sablier dure 15 seconde
            mainClock.tick(60)                  # on compense le potentiel manque de fps
            pygame.display.update()
        ...
        while i < 800:
            ...
            objets.texte("Solution", 40, (0,0,0), screen, "center", 55, font)
            objets.texte(question["q"], 30, (0,0,0), screen, "center", 149, font)
            objets.texte(question["r"], 30, (0,0,0), screen, "center", 221, font) # affichage de la question et sa reponse
            ...
            i += (800/(5*mainClock.get_fps())) # sablier de 5 secondes
            mainClock.tick(60)
            pygame.display.update()

    # une fois toute les questions passees
    fin_jeu()
...
def fin_jeu():
    ...
    pygame.display.set_caption('Clash Turtle - Scores')
    screen.fill(fond_jeu)
    objets.texte("Scores", 40, (0,0,0), screen, "center", 53, font)
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:                 # commandes relatives aux touches
                if event.key == K_ESCAPE:
                    pygame.mixer.music.load(data_carte_selectionnee["musique"]) # lancer le theme de la carte selectionne
                    pygame.mixer.music.play(-1)
                    menu_principal()
            if event.type == MOUSEBUTTONDOWN:         # commandes relatives a la souris
                if event.button == 1:
        ...
        if click:
            ...
            if bouton_rejouer.collidepoint((mx, my)):
                tirage_couleurs() # relancer un tirage de la couleur des joueurs
                if joueur_selectionne == j2:
                    pre_jeu()
                else:
                    jeu()
